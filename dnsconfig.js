var cloudflare = NewDnsProvider("cloudflare", "CLOUDFLAREAPI");
var REG_CHANGEME = NewRegistrar("ThirdParty", "NONE");

var DEFAULT = [
  CAA_BUILDER({
    label : "*",
    iodef : "mailto:admin@cxsrv.de",
    issue : [ "letsencrypt.org" ],
    issuewild : [ "letsencrypt.org" ],
  }),
  CAA_BUILDER({
    label : "@",
    iodef : "mailto:admin@cxsrv.de",
    issue : [ "letsencrypt.org" ],
    issuewild : [ "letsencrypt.org" ],
  }),
];

var MAIL01 = [
  MX('@', 10, 'mail.cxsrv.de.'),
  CNAME('autoconfig', 'mail.cxsrv.de.'),
  CNAME('autodiscover', 'mail.cxsrv.de.'),
  CNAME('webmail', 'mail.cxsrv.de.'),
  TXT('_caldavs._tcp', 'path=/SOGo/dav/'),
  TXT('_carddavs._tcp', 'path=/SOGo/dav/'),
  SRV('_autodiscover._tcp', 0, 1, 443, 'mail.cxsrv.de.'),
  SRV('_autodiscover._tcp', 0, 0, 443, 'mail.cxsrv.de.'),
  SRV('_caldavs._tcp', 0, 1, 443, 'mail.cxsrv.de.'),
  SRV('_carddavs._tcp', 0, 1, 443, 'mail.cxsrv.de.'),
  SRV('_imaps._tcp', 0, 1, 993, 'mail.cxsrv.de.'),
  SRV('_imap._tcp', 0, 1, 143, 'mail.cxsrv.de.'),
  SRV('_pop3s._tcp', 0, 1, 995, 'mail.cxsrv.de.'),
  SRV('_pop3._tcp', 0, 1, 110, 'mail.cxsrv.de.'),
  SRV('_sieve._tcp', 0, 1, 4190, 'mail.cxsrv.de.'),
  SRV('_smtps._tcp', 0, 1, 465, 'mail.cxsrv.de.'),
  SRV('_submission._tcp', 0, 1, 587, 'mail.cxsrv.de.'),
];

// Backend

D("cxsrv.de", REG_CHANGEME, DnsProvider(cloudflare), DEFAULT, MAIL01,
  A('data2', '159.69.203.63'), A('dh1', '94.130.75.68'),
  A('mail', '157.90.168.57'), A('voice', '162.55.60.38'),
  AAAA('data2', '2a01:4f8:1c1c:5b1d::1'), AAAA('dh1', '2a01:4f8:c0c:e911::1'),
  AAAA('mail', '2a01:4f8:1c1c:4a9d::1'), AAAA('voice', '2a01:4f8:1c1c:9680::1'),
  TLSA('_25._tcp.mail', 3, 1, 1,
       '8dd735fc435585f048b12b576cf11a89187e820b3a57ff72d1172f9a6bdad128'),
  TXT('@', 'v=spf1 mx -all'),
  TXT('dkim._domainkey',
      'v=DKIM1;k=rsa;t=s;s=email;p='),
  TXT('_dmarc',
      'v=DMARC1; p=reject; rua=mailto:admin@cxsrv.de; ruf=mailto:admin@cxsrv.de;'))

// Projektdomains

D("rsmg-clan.de", REG_CHANGEME, DnsProvider(cloudflare), DEFAULT, MAIL01,
  A('backup', '148.251.152.30'), A('data', '159.69.203.63'),
  AAAA('backup', '2a01:4f8:210:5126::1'), AAAA('data', '2a01:4f8:1c1c:5b1d::1'),
  CNAME('amongus', 'dh1.cxsrv.de.'), CNAME('cloud', 'data.rsmg-clan.de.'),
  CNAME('crewlink', 'voice.cxsrv.de.'), CNAME('git', 'data.rsmg-clan.de.'),
  CNAME('mail', 'mail.cxsrv.de.'), CNAME('music', 'voice.cxsrv.de.'),
  CNAME('pma', 'dh1.cxsrv.de.'), CNAME('ts3', 'voice.cxsrv.de.'),
  CNAME('vpn', 'dh1.cxsrv.de.'),
  SRV('_ts3._udp.ts3', 1, 1, 9987, 'ts3.rsmg-clan.de.'),
  TXT('dkim._domainkey',
      'v=DKIM1;k=rsa;t=s;s=email;p='),
  TXT('_dmarc',
      'v=DMARC1; p=reject; rua=mailto:admin@rsmg-clan.de; ruf=mailto:admin@rsmg-clan.de;'),
  TXT('@', 'v=spf1 mx -all'))
